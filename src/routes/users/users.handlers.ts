...
export const list: AppRouteHandler<ListRoute> = async (c) => {
  ...
  const currentUser = await db.query.users.findFirst({
    where(fields, operators) {
      return operators.eq(fields.id, jwtPayload.id);
    },
  });
  ...
  let usersQuery: InferSelectModel<typeof users>[] = [];
    ...
    usersQuery = await db.query.users.findMany({
      where(fields, operators) {
        return operators.and(
          ...
          operators.eq(fields.workspaceId, currentUser.workspaceId),
        );
      },
    });
  }

  return c.json(sanitizeUsers(usersQuery));
};
